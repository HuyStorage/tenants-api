package com.tenant.api.jwt;

import com.tenant.api.utils.ZipUtils;
import lombok.Data;
...
@Data
@Slf4j
public class TenantJwt implements Serializable {
    ...
    public static TenantJwt decode(String input) {
        TenantJwt result = null;
        try {
            String[] items = ZipUtils.unzipString(input).split(DELIM, 11);
            if (items.length >= 10) {
                result = new TenantJwt();

...
public class OTPService {
    ...
    public void storeOtp(String email, String otp, String tenantId) {
        String key = redisService.buildKey(tenantId, email);
        redisService.put(key, otp, FIVE_MINUTES);
    }

    public boolean verifyOtp(String email, String otp, String tenantId) {
        String key = redisService.buildKey(tenantId, email);
        String cachedOtp = redisService.get(key, String.class);
        return Objects.equals(otp, cachedOtp);
    }

    public void deleteOtp(String email, String tenantId) {
        String key = redisService.buildKey(tenantId, email);
        redisService.deleteByPrefix(key);
    }

    public synchronized String resendOtp(String email, String tenantId) {
        String key = redisService.buildKey(tenantId, email, "resend");
        ...
        this.storeOtp(email, otp, tenantId);
